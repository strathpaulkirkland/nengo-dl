...
class Converter:
    ...
    net : `nengo.Network`
        The converted Nengo network. The network will have ``.inputs`` and
        ``.outputs`` dictionaries which can be used to look up the Nengo equivalents of
        ``model.inputs`` and ``model.outputs`` (e.g., ``net.outputs[model.outputs[0]]``
        will be the output `~nengo.Probe` corresponding to that output of the Keras
        model).
    """
    ...
    def verify(self, training=False, inputs=None):
        ...
        print("Output of Nengo network matches Keras model")
...
class ConvertModel(LayerConverter):
    ...
    def trace_tensors(self, tensors, results=None):
        ...
        results : list of ``tf.Tensor``
            Output tensors for all the layers leading up and including ``tensors``.
