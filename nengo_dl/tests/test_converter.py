...
def _test_convert(inputs, outputs, allow_fallback=False, inp_vals=None):
    ...
    conv.verify(training=False, inputs=inp_vals)
...
def test_batch_normalization(rng):
    ...
    #
    # conv.verify(model, net, training=False, inputs=inp_vals)
    # conv.verify(model, net, training=True, inputs=inp_vals)

    # test actually converting to nengo objects
    conv = converter.Converter(model, allow_fallback=False, freeze_batchnorm=True)

    conv.verify(training=False, inputs=inp_vals)
    with pytest.raises(ValueError, match="does not match output of converted"):
...
def test_densenet(seed):
    ...
    # note: just checking that the conversion passes (i.e. the network is fully
    # converted); we don't expect any of the verification checks to pass, due to the
    # max_to_avg_pool swap
